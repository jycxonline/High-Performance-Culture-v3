...
from hpc.gamification.pages import (
    page_pace, page_submit_action_plan, page_checkpoint_update, page_pace_admin,
)
...
st.markdown(f"""<style>
    ...
    h1, h2, h3 {{ color: {NAVY}; }}
    .stButton>button {{ background-color: {NAVY}; color: white; border: 0; padding: 0.5rem 1.2rem;
        border-radius: 6px; font-weight: 600; }}
    .stButton>button:hover {{ background-color: #2A4A7F; color: white; }}
    .hpc-hero {{ background: linear-gradient(135deg, {NAVY} 0%, #2A4A7F 100%); color: white;
        padding: 1.5rem 2rem; border-radius: 10px; margin-bottom: 1.4rem; border-bottom: 3px solid {GOLD}; }}
    .hpc-hero h1 {{ color: white; margin: 0; font-size: 1.7rem; }}
    ...
    .focus-strip .fk {{ font-size:0.92rem; color:#33415C; }}
    .rec-card {{ border:1px solid #E3E8F0; border-left:5px solid var(--pc); border-radius:8px;
        padding:11px 15px; margin-bottom:11px; background:#FCFDFF; }}
    .rec-card .el {{ font-weight:700; color:{NAVY}; font-size:0.98rem; }}
    .rec-card .pri {{ float:right; font-weight:700; font-size:0.76rem; }}
    .pol-card {{ border-left:4px solid {NAVY}; background:#F4F7FC; padding:10px 14px; margin-bottom:10px;
        border-radius:6px; }}
    .sec-title {{ font-size:1.15rem; font-weight:700; color:{NAVY}; margin:8px 0 6px; }}
</style>""", unsafe_allow_html=True)

_PRI = {"Critical": "#C0392B", "High": "#ED7D31", "Medium": "#3F7D3A", "Low": "#8C8C8C"}
...
def get_analysis_for_dept(dept_name):
    try:
        ...
        if len(df) == 0 or dept_name not in df["Department"].unique(): return None
...
with st.sidebar:
    ...
    page = st.radio("Nav",
        ["🏠 Home", "📝 Take Questionnaire", "📊 Admin Dashboard", "📄 Generate Executive Report",
         "📥 Upload Response Data", "⚙️ Upload Configuration", "———",
         "🏃 PACE", "🎛️ Submit Action Plan", "📍 Checkpoint Update", "🛠️ PACE Admin"],
    ...
    try:
        st.caption(f"**Submissions:** {get_responses()['Submission ID'].nunique()}")
    except Exception: pass
...
def page_home():
    ...
    c1.metric("Submissions", f"{df['Submission ID'].nunique():,}")
    c2.metric("Departments", df["Department"].nunique())
    c3.metric("Company PACE", f"{co:.2f}", f"{imb:.2f} gap")
    c4.markdown(f"**Classification**<br>{band_chip(classify_score(co, cfg))}", unsafe_allow_html=True)
    st.dataframe(pd.DataFrame({"Element": PILLARS, "Score": [round(float(cp[p]), 2) for p in PILLARS],
...
def page_questionnaire():
    ...
    if st.session_state.quest_step == "start":
        ...
        if st.button("Start", type="primary", disabled=(dept == "")):
            st.session_state.quest_dept = dept; st.session_state.quest_step = "answer"
            st.session_state.quest_answers = {}; st.rerun()
        return
    ...
    if st.session_state.quest_step == "answer":
        ...
        for tab, pillar in zip(tabs, PILLARS):
            with tab:
                ...
                for _, row in active[active["Pillar"] == pillar].iterrows():
                    ...
                    val = st.radio(f"**{qid}.** {row['Question Text']}", opts, index=idx, horizontal=True, key=f"q_{qid}")
                    if val is not None: st.session_state.quest_answers[qid] = val
                    st.markdown("---")
        answered = sum(1 for _, r in active.iterrows() if st.session_state.quest_answers.get(r["Question ID"]) is not None)
        ...
        with c3:
            if st.button("Submit ✓", type="primary", disabled=(answered != total)):
                ...
                answers = {r["Question ID"]: (r["Question Text"], r["Pillar"], int(st.session_state.quest_answers[r["Question ID"]]))
                ...
                clear_caches(); st.session_state.quest_step = "done"; st.session_state.quest_sub_id = sub_id; st.rerun()
...
def _ensure_response_file():
    ...
    for j, h in enumerate(["Submission ID", "Submission Timestamp", "Department", "Respondent ID",
        ...
        c.fill = PatternFill("solid", start_color="1F3864"); c.alignment = Alignment(horizontal="center")
...
def page_dashboard():
    ...
    st.markdown(
        f"<div class='focus-strip'>"
        f"<span class='fname'>Focus: {analysis.focus.department}</span>"
        f"<span class='fk'>Respondents: <b>{analysis.focus.n_respondents:,}</b></span>"
        f"<span class='fk'>PACE score: <b>{analysis.focus.overall:.2f}</b></span>"
    ...
    for i, t in enumerate(analysis.tailored_recommendations):
        col = _PRI.get(t["Priority"], "#8C8C8C")
        with tcols[i % 2]:
            st.markdown(f"<div class='rec-card' style='--pc:{col}'>"
    ...
    with st.expander("📊 Department ranking (all departments)"):
        _safe_chart(charts.ranking_bar, analysis.all_departments, analysis.company_overall,
                    analysis.focus.department if analysis.focus.department != "Company-wide" else None)
...
def page_upload_responses():
    ...
    if up is not None:
        ...
        try: incoming = load_responses(str(tmp))
        except Exception as e: st.error(f"Rejected: {e}"); tmp.unlink(missing_ok=True); return
        st.success(f"✅ {len(incoming):,} rows."); st.dataframe(incoming.head(10), use_container_width=True)
        if st.button("Apply ✓", type="primary"):
            if mode == "Replace": shutil.copy(tmp, RESPONSE_PATH)
            else:
...
def page_upload_config():
    st.markdown(f"""<div class="hpc-hero"><h1>Upload Configuration</h1><p>Edit config Excel then upload.</p></div>""", unsafe_allow_html=True)
    current = get_config()
    ...
    if up is not None:
        tmp = DATA_DIR / f"_incoming_{up.name}"; tmp.write_bytes(up.getvalue())
        try: incoming = load_config(str(tmp))
        except Exception as e: st.error(f"❌ {e}"); tmp.unlink(missing_ok=True); return
        st.success("✅ Valid.")
        diff = diff_configs(current, incoming)
        for cat, items in diff.items():
            if items:
                with st.expander(f"{cat} ({len(items)})", expanded=len(items) < 6):
                    for item in items: st.markdown(f"- {item}")
        changed_by = st.text_input("Changed by"); change_type = st.selectbox("Type", ["Update", "Question edit", "Dept edit", "Other"])
...
elif page == "🎛️ Submit Action Plan": page_submit_action_plan(str(LOOKUP_PATH), str(ACTION_PLAN_DIR), get_analysis_for_dept)
elif page == "📍 Checkpoint Update": page_checkpoint_update(str(LOOKUP_PATH), str(ACTION_PLAN_DIR), get_analysis_for_dept)
