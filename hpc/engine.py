...
class PolarisationStat:
    ...
    std: float
    variance: float
    pct_low: float          # % of scores <= 4
    pct_mid: float          # % of scores 5-6
    pct_high: float         # % of scores >= 7
    polarised: bool
    variance_label: str     # Low / Moderate / High variance
    statement: str          # written analysis
...
# ---------------------------------------------------------------------------
# Polarisation & Variance Analysis (per element / pillar)
# ---------------------------------------------------------------------------
def _polarisation_for_pillar(pillar: str, scores: np.ndarray) -> PolarisationStat:
    ...
    if std >= 2.0:
        vlabel = "High variance"
    elif std >= 1.3:
        vlabel = "Moderate variance"
    else:
        vlabel = "Low variance"
    ...
    # Written statement
    if polarised:
        stmt = (f"{pillar} is <b>polarised</b> — the team is split: {pct_low:.0f}% rate it low "
                f"(&le;4) while {pct_high:.0f}% rate it high (&ge;7). {vlabel} (SD={std:.2f}) confirms "
                f"inconsistent experiences that a single average masks. Investigate sub-teams before acting.")
    elif pct_high >= 60:
        stmt = (f"{pillar} shows <b>broad agreement it is a strength</b> — {pct_high:.0f}% rate it "
                f"high (&ge;7) with {vlabel.lower()} (SD={std:.2f}). Protect and codify what works.")
    elif pct_low >= 45:
        stmt = (f"{pillar} shows <b>broad concern</b> — {pct_low:.0f}% rate it low (&le;4), "
                f"{vlabel.lower()} (SD={std:.2f}). This is a consistent, shared issue, not an outlier.")
    elif std >= 2.0:
        stmt = (f"{pillar} has <b>high variance</b> (SD={std:.2f}) without a clear split — views are "
                f"scattered ({pct_low:.0f}% low, {pct_mid:.0f}% mid, {pct_high:.0f}% high), suggesting "
                f"uneven experiences across roles or teams.")
    else:
        stmt = (f"{pillar} shows a <b>settled, consensual view</b> (SD={std:.2f}); "
                f"{pct_high:.0f}% high, {pct_low:.0f}% low. The average is a reliable summary here.")
...
def analyze(responses: pd.DataFrame, focus_dept: str, cfg: HPCConfig) -> AnalysisResult:
    ...
    if focus_dept == "__ALL__":
        ...
        if focus_dept not in dept_pillar.index:
            raise ValueError(f"Department '{focus_dept}' not in data.")
    ...
    # Polarisation & variance per pillar (from raw item scores)
    polarisation = []
    for p in PILLARS:
...
def _gen_insights(focus, dept_pillar, imb_by_dept, company_pillar, cfg):
    ...
    ins.append({"label": "Strongest element", "text": f"{strong} ({means[strong]:.2f}) — {gv[strong]:+.2f} vs. company.", "rule": "R-01"})
    ins.append({"label": "Weakest element", "text": f"{weak} ({means[weak]:.2f}) — {gv[weak]:+.2f} vs. company.", "rule": "R-02"})
    largest = min(gv, key=gv.get)
    ins.append({"label": "Largest gap vs. company", "text": f"{largest}: {gv[largest]:+.2f}.", "rule": "R-03"})
    if focus.imbalance > cfg.imbalance_moderate_max:
        ins.append({"label": "Imbalance flag", "text": f"Gap {focus.imbalance:.2f} exceeds threshold — classification downgraded.", "rule": "R-04"})
    elif focus.imbalance > cfg.imbalance_wellbalanced_max:
        ins.append({"label": "Imbalance watch", "text": f"Gap {focus.imbalance:.2f} — moderate; watch item.", "rule": "R-04"})
    else:
        ins.append({"label": "Balance", "text": f"Gap {focus.imbalance:.2f} — well balanced.", "rule": "R-04"})
    if len(dept_pillar) > 1:
        ins.append({"label": "Most balanced dept", "text": f"{imb_by_dept.index[0]} (gap {imb_by_dept.iloc[0]:.2f}).", "rule": "R-05"})
        ins.append({"label": "Most imbalanced dept", "text": f"{imb_by_dept.index[-1]} (gap {imb_by_dept.iloc[-1]:.2f}).", "rule": "R-06"})
    return ins
...
ACTION_LIBRARY = {
    ...
    "Collaboration": [
        ("Launch a 60-day workflow rescue on the top-3 cross-team bottlenecks.", "Critical",
         "Move Collaboration by 0.6–1.0; cut hand-off delays.", "Head of Dept + COO sponsor", "0–90 days"),
...
def _gen_tailored(focus, polarisation, company_pillar):
    """New: tailored actionable recommendations driven by score + polarisation/variance."""
    ...
    for pillar, mean in ordered:
        ...
        # Choose the recommendation flavour based on the pattern
        if pol and pol.polarised:
            ...
            rationale = f"Polarised (SD={pol.std:.2f}) — a blanket action risks alienating one half of the team."
            priority = "High"
        elif mean < 5.5:
            action = f"Prioritise a focused improvement sprint on {pillar} — it is the clearest drag on performance."
            ...
            action = f"Codify and share what makes {pillar} work — turn it into a repeatable playbook for other teams."
            rationale = f"Consistent strength ({mean:.2f}, {pol.pct_high:.0f}% high) worth scaling."
            priority = "Medium"
        elif pol and pol.std >= 2.0:
            action = f"Segment {pillar} results by role/sub-team — scattered views ({pol.variance_label.lower()}) hide pockets of risk."
        ...
        out.append({"Element": pillar, "Mean": f"{mean:.2f}", "Gap": f"{gap:+.2f}",
                    "Recommendation": action, "Why (data-driven)": rationale, "Priority": priority})
    return out
