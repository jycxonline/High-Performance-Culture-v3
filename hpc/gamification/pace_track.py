"""
PACE journey — straight-line running-track roadmap.
...
def render_pace_journey(dept, medal_override=None):
    ...
    # Runner/label = the department (name in header, code on the runner bubble)
    ...
    if stage_idx > 0 and stage_idx < n:
        ...
        ax.text(xs[-1] + 3.5, y + 8.4, dept_label, fontsize=8, ha="center", va="center",
                color=WHITE, fontweight="bold", zorder=13,
                bbox=dict(boxstyle="round,pad=0.3", facecolor=runner_hex, edgecolor=WHITE, linewidth=0.9))
    ...
    ax.text(50, 2.9, f"Current stage:  {dept.stage_display}", fontsize=9, color=WHITE,
            fontweight="bold", va="center", zorder=15)
    ax.text(50, 1.3, f"Department:  {dept_label}  ·  {dept.runner_colour}",
