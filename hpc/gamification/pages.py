...
def page_pace(lookup_path, action_plan_dir):
    ...
    with c1:
        ...
        rows = [("Code", dept.dept_code),
                ("Runner colour", dept.runner_colour),
...
def page_submit_action_plan(lookup_path, action_plan_dir, get_analysis_for_dept):
    ...
    with st.form(f"ap_{dept.dept_id}"):
        ...
        for i in range(3):
            ...
            a = st.text_input(f"Description ({i+1})", value=existing_actions[i].get("action", ""), key=f"act_{dept.dept_id}_{i}")
            cc1, cc2 = st.columns(2)
            with cc1:
                o = st.text_input(f"Owner ({i+1})", value=existing_actions[i].get("owner", ""), key=f"own_{dept.dept_id}_{i}")
            with cc2:
                t = st.text_input(f"Timeline ({i+1})", value=existing_actions[i].get("timeline", ""), key=f"tim_{dept.dept_id}_{i}")
        ...
        if st.form_submit_button("📨 Submit action plan", type="primary"):
            plan = {"dept_id": dept.dept_id, "dept_name": dept.dept_name,
                    "insight": insight.strip(), "priority_pillar": pillar, "commitment": commitment.strip(),
            ...
            if not insight or not commitment or not plan["actions"]:
                ...
                st.success("✅ Action plan submitted."); st.balloons()
                ...
                with c1:
                    ...
                    if new_actual != dept.actual:
                        if st.button("Save", key=f"save_{dept.dept_id}"):
                            ...
                            log_admin_action(lookup_path, admin_user, "Updated responses", dept.dept_name, f"→ {dept.actual}")
                            st.rerun()
                with c2:
                    st.markdown("**Progression**")
                ...
                with c3:
                    ...
                    if not dept.report_reviewed:
                        ...
                        if st.button("✅ Sign off CP1", key=f"cp1_{dept.dept_id}"):
                            dept = advance_stage(dept, "mark_checkpoint_1"); save_lookup(lookup_path, [dept])
                            log_admin_action(lookup_path, admin_user, "CP1", dept.dept_name, ""); st.rerun()
                    elif not dept.checkpoint_2:
                        if st.button("✅ Sign off CP2", key=f"cp2_{dept.dept_id}"):
                            dept = advance_stage(dept, "mark_checkpoint_2"); save_lookup(lookup_path, [dept])
                            log_admin_action(lookup_path, admin_user, "CP2", dept.dept_name, ""); st.rerun()
                    elif not dept.checkpoint_3:
                        if st.button("✅ Sign off CP3", key=f"cp3_{dept.dept_id}"):
                            dept = advance_stage(dept, "mark_checkpoint_3"); save_lookup(lookup_path, [dept])
                            log_admin_action(lookup_path, admin_user, "CP3", dept.dept_name, ""); st.rerun()
                        ...
                        if st.button(f"🏁 Finish Line → {medal}", key=f"dst_{dept.dept_id}", type="primary"):
                            ...
                            dept = advance_stage(dept, "assign_destination"); save_lookup(lookup_path, [dept])
    ...
    with tab4:
        if not Path(lookup_path).exists():
            ...
            funnel = pd.DataFrame({
                ...
                "Departments": [total, training, sum(1 for d in depts if d.report_reviewed), implementing,
                    sum(1 for d in depts if d.checkpoint_1), sum(1 for d in depts if d.checkpoint_2), cp3, finished]})
