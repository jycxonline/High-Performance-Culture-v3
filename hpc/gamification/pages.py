...
from .engine import (
    ACTIVATION_THRESHOLD, COLOUR_HEX, DepartmentJourney,
    load_lookup, save_lookup, compute_stage, advance_stage,
    badges_earned, checkpoint_challenges,
...
def _hero(title, subtitle):
    ...
    <p style="color:#D9E2F3;margin:0.4rem 0 0 0;font-size:0.95rem">{subtitle}</p>
    </div>""", unsafe_allow_html=True)


def _dept_picker(depts, key="dept_picker"):
    if not depts:
        st.info("No departments found in the Department Lookup file.")
        return None
...
def page_pace(lookup_path, action_plan_dir):
    ...
    if not Path(lookup_path).exists():
        st.error("Department Lookup file not found. Upload one via PACE Admin.")
        return
    ...
    if dept is None:
        ...
        total = len(depts)
        training = sum(1 for d in depts if d.completion_pct >= ACTIVATION_THRESHOLD)
        implementing = sum(1 for d in depts if d.action_plan_submitted)
        finished = sum(1 for d in depts if d.destination_assigned)
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Total departments", total)
        c2.metric("In Training (≥70%)", training)
        c3.metric("Implementing change", implementing)
        c4.metric("Finished", finished)
        st.info("👆 Select a department above to see its individual PACE journey.")
        rows = [{"Code": d.dept_code, "Department": d.dept_name,
                  "Completion %": f"{d.completion_pct:.0%}",
                  "Current stage": d.stage_display, "Badges": f"{len(badges_earned(d))} / 9"} for d in depts]
        st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
        return

    st.markdown("### PACE journey")
    fig = render_pace_journey(dept)
    ...
    with c1:
        ...
        rows = [
            ("Code", dept.dept_code),
            ("Runner", f"{dept.runner_type} · {dept.runner_colour}"),
    ...
    with c2:
        ...
        flags = [
            ("Report Reviewed", dept.report_reviewed),
            ("Action Plan Submitted", dept.action_plan_submitted),
            ("Checkpoint 1", dept.checkpoint_1),
            ("Checkpoint 2", dept.checkpoint_2),
            ("Checkpoint 3", dept.checkpoint_3),
            ("Finish Line", dept.destination_assigned),
        ]
        for i, (name, val) in enumerate(flags):
            with (cc1 if i < 3 else cc2):
        ...
        if not earned:
            ...
            html = " ".join(
                ...
                f'border-radius:12px;font-size:0.8rem;background:{GOLD};color:white;'
                f'font-weight:600">🏅 {b}</span>' for b in earned)
...
def page_submit_action_plan(lookup_path, action_plan_dir, get_analysis_for_dept):
    _hero("Implement Change — Action Plan",
          "Translate your PACE diagnostic report into an action plan.")
    if not Path(lookup_path).exists():
    ...
    if dept is None:
        st.info("👆 Select your department above."); return

    weakest = None
    try:
        analysis = get_analysis_for_dept(dept.dept_name)
        if analysis is not None:
            weakest = min(analysis.focus.pillar_means, key=analysis.focus.pillar_means.get)
    except Exception:
        pass
    ...
    <b>Department:</b> {dept.dept_name} ({dept.dept_code})<br>
    <b>Current stage:</b> {dept.stage_display}<br>
    <b>Completion:</b> {dept.actual}/{dept.expected} ({dept.completion_pct:.0%})
    {'<br><b>Priority element:</b> ' + weakest if weakest else ''}
    </div>""", unsafe_allow_html=True)
    ...
    with st.form(f"ap_{dept.dept_id}"):
        ...
        pillar = st.selectbox("2. Priority element", pillars,
                                index=pillars.index(default_p) if default_p in pillars else 0)
        ...
        for i in range(3):
            ...
            a = st.text_input(f"Description ({i+1})", value=existing_actions[i].get("action", ""),
                                key=f"act_{dept.dept_id}_{i}")
            cc1, cc2 = st.columns(2)
            with cc1:
                o = st.text_input(f"Owner ({i+1})", value=existing_actions[i].get("owner", ""),
                                    key=f"own_{dept.dept_id}_{i}")
            with cc2:
                t = st.text_input(f"Timeline ({i+1})", value=existing_actions[i].get("timeline", ""),
                                    key=f"tim_{dept.dept_id}_{i}")
            m = st.text_input(f"Success measure ({i+1})", value=existing_actions[i].get("success_measure", ""),
                                key=f"suc_{dept.dept_id}_{i}")
            new_actions.append({"action": a, "owner": o, "timeline": t, "success_measure": m})
        ...
        if st.form_submit_button("📨 Submit action plan", type="primary"):
            plan = {"dept_id": dept.dept_id, "dept_name": dept.dept_name,
                    "insight": insight.strip(), "priority_pillar": pillar,
                    "commitment": commitment.strip(),
                    "actions": [a for a in new_actions if a.get("action", "").strip()],
                    "support_required": support.strip(),
            ...
            if not insight or not commitment or not plan["actions"]:
                ...
                save_action_plan(action_plan_dir, dept.dept_id, plan)
                st.success("✅ Action plan submitted.")
                st.balloons()
...
def page_checkpoint_update(lookup_path, action_plan_dir, get_analysis_for_dept):
    ...
    if not eligible:
        st.info("No departments have started implementing change yet."); return
    dept = _dept_picker(eligible, key="cp_picker")
    if dept is None:
        st.info("👆 Select your department."); return
    ...
    if not dept.checkpoint_1: next_cp = 1
        ...
        st.success("🎉 All three checkpoints complete — ready for the finish line."); return

    cp_name = list(challenges.keys())[next_cp - 1]
    cp = challenges[cp_name]
    ...
    if cp.get("priority_hint"):
        ...
        border-radius:4px;margin:0.8rem 0;font-size:0.9rem">💡 <b>Priority insight:</b> {cp['priority_hint']}</div>""",
    ...
    with st.form(f"cp_{dept.dept_id}_{next_cp}"):
        ...
        if st.form_submit_button(f"📨 Submit Checkpoint {next_cp}", type="primary"):
            if not text.strip():
                st.error("Please write your update.")
            else:
                save_checkpoint_update(action_plan_dir, dept.dept_id, next_cp, text.strip())
                st.success(f"✅ Checkpoint {next_cp} submitted.")
                st.balloons()
...
def page_pace_admin(lookup_path, action_plan_dir, data_dir):
        ...
        with c1:
            if Path(lookup_path).exists():
                with open(lookup_path, "rb") as f:
                    st.download_button("⬇️ Download current lookup", f.read(),
                                        file_name="PACE_Department_Lookup.xlsx")
            else:
                st.warning("No lookup file loaded.")
        with c2:
            up = st.file_uploader("Upload new lookup", type=["xlsx"])
            if up is not None:
                tmp = Path(data_dir) / f"_incoming_lookup_{up.name}"
                tmp.write_bytes(up.getvalue())
                try:
                    incoming = load_lookup(str(tmp))
                    st.success(f"✅ Valid. {len(incoming)} depts.")
                    if st.button("Apply ✓", type="primary"):
                        backup = Path(data_dir) / f"PACE_Department_Lookup.backup.{datetime.now().strftime('%Y%m%d%H%M%S')}.xlsx"
                        if Path(lookup_path).exists():
                            shutil.copy(lookup_path, backup)
                        shutil.copy(tmp, lookup_path)
                        tmp.unlink(missing_ok=True)
                        log_admin_action(lookup_path, admin_user, "Uploaded lookup", "All", f"{len(incoming)} depts")
                        st.success(f"✅ Updated. Backup: {backup.name}")
                        st.rerun()
    ...
    with tab2:
        if not Path(lookup_path).exists():
            st.warning("Upload a lookup file first.")
        else:
            depts = load_lookup(lookup_path)
            for d in depts: d.stage = compute_stage(d)
            dept = _dept_picker(depts, key="admin_picker")
            if dept is None:
                st.info("👆 Select a department.")
            else:
                ...
                with c1:
                    ...
                    new_actual = st.number_input("Update responses", min_value=0,
                                                    value=dept.actual, key=f"act_{dept.dept_id}")
                    if new_actual != dept.actual:
                        if st.button("Save", key=f"save_{dept.dept_id}"):
                            dept.actual = int(new_actual)
                            dept.completion_pct = (dept.actual / dept.expected) if dept.expected else 0
                            dept.stage = compute_stage(dept)
                            save_lookup(lookup_path, [dept])
                            log_admin_action(lookup_path, admin_user, "Updated responses",
                                              dept.dept_name, f"→ {dept.actual}")
                            st.rerun()
                with c2:
                    st.markdown("**Progression**")
                    st.markdown(f"{'✅' if dept.report_reviewed else '⬜'} Report Reviewed")
                    st.markdown(f"{'✅' if dept.action_plan_submitted else '⬜'} Action Plan")
                    st.markdown(f"{'✅' if dept.checkpoint_1 else '⬜'} Checkpoint 1")
                    st.markdown(f"{'✅' if dept.checkpoint_2 else '⬜'} Checkpoint 2")
                    st.markdown(f"{'✅' if dept.checkpoint_3 else '⬜'} Checkpoint 3")
                    st.markdown(f"{'✅' if dept.destination_assigned else '⬜'} Finish Line")
                with c3:
                    st.markdown("**Next admin action**")
                    if not dept.report_reviewed:
                        if st.button("✅ Mark Report Reviewed", key=f"rep_{dept.dept_id}"):
                            dept = advance_stage(dept, "mark_report_reviewed")
                            save_lookup(lookup_path, [dept])
                            log_admin_action(lookup_path, admin_user, "Report reviewed", dept.dept_name, "")
                            st.rerun()
                        ...
                        if plan:
                            ...
                            if st.button("🏃 Approve → Implement", key=f"apr_{dept.dept_id}", type="primary"):
                                dept = advance_stage(dept, "approve_action_plan")
                                save_lookup(lookup_path, [dept])
                                log_admin_action(lookup_path, admin_user, "Approved plan", dept.dept_name, "")
                                st.rerun()
                        else:
                            st.caption("Awaiting plan submission.")
                    elif not dept.checkpoint_1:
                        if st.button("✅ Sign off CP1", key=f"cp1_{dept.dept_id}"):
                            dept = advance_stage(dept, "mark_checkpoint_1")
                            save_lookup(lookup_path, [dept])
                            log_admin_action(lookup_path, admin_user, "CP1", dept.dept_name, "")
                            st.rerun()
                    elif not dept.checkpoint_2:
                        if st.button("✅ Sign off CP2", key=f"cp2_{dept.dept_id}"):
                            dept = advance_stage(dept, "mark_checkpoint_2")
                            save_lookup(lookup_path, [dept])
                            log_admin_action(lookup_path, admin_user, "CP2", dept.dept_name, "")
                            st.rerun()
                    elif not dept.checkpoint_3:
                        if st.button("✅ Sign off CP3", key=f"cp3_{dept.dept_id}"):
                            dept = advance_stage(dept, "mark_checkpoint_3")
                            save_lookup(lookup_path, [dept])
                            log_admin_action(lookup_path, admin_user, "CP3", dept.dept_name, "")
                            st.rerun()
                        ...
                        if st.button(f"🏁 Finish Line → {medal}", key=f"dst_{dept.dept_id}", type="primary"):
                            dept.destination = medal
                            dept = advance_stage(dept, "assign_destination")
                            save_lookup(lookup_path, [dept])
                            log_admin_action(lookup_path, admin_user, "Finish line", dept.dept_name, medal)
                            st.rerun()
                    else:
                        st.success("🏆 PACE Setter — finished!")

                st.markdown("---")
                st.markdown("### PACE journey preview")
                fig = render_pace_journey(dept)
                st.pyplot(fig, use_container_width=True)
            ...
            if plan:
                ...
                with st.expander(f"{d.dept_code} — {d.dept_name} · {approved}"):
                    ...
                    for i, a in enumerate(plan.get("actions", []), start=1):
                        st.markdown(f"{i}. **{a.get('action','')}** — Owner: {a.get('owner','—')} · "
                                    f"Timeline: {a.get('timeline','—')}")
                    st.caption(f"Submitted at {plan.get('submitted_at','')}")
        if not found:
            st.info("No plans submitted yet.")

    with tab4:
        if not Path(lookup_path).exists():
            st.warning("Upload lookup first.")
        else:
            depts = load_lookup(lookup_path)
            for d in depts: d.stage = compute_stage(d)
            total = len(depts)
            training = sum(1 for d in depts if d.completion_pct >= ACTIVATION_THRESHOLD)
            implementing = sum(1 for d in depts if d.action_plan_submitted)
            cp3 = sum(1 for d in depts if d.checkpoint_3)
            finished = sum(1 for d in depts if d.destination_assigned)
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("In Training", training, f"of {total}")
            c2.metric("Implementing", implementing, f"of {total}")
            c3.metric("Race ready", cp3, f"of {total}")
            c4.metric("Finished", finished, f"of {total}")
            funnel = pd.DataFrame({
                "PACE Stage": ["Warm-up", "Training", "Reflect", "Implement Change",
                               "Reflect (2)", "Training-Tempo", "Race up PACE", "Finish Line"],
                "Departments": [total, training,
                    sum(1 for d in depts if d.report_reviewed), implementing,
                    sum(1 for d in depts if d.checkpoint_1),
                    sum(1 for d in depts if d.checkpoint_2), cp3, finished],
            })
