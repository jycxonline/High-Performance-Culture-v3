...
def radar_chart(focus, company, label="Selected", figsize=(5.6, 5.0)):
    """Outline radar so BOTH series are visible."""
    labels = PILLARS
...
def polarisation_bar(polarisation, figsize=(5.8, 4.2)):
    """Robust: plain floats, ASCII labels, getattr guards."""
    fig, ax = plt.subplots(figsize=figsize)
    pillars = [str(getattr(p, "pillar", "")) for p in polarisation]
    low = [float(getattr(p, "pct_low", 0.0) or 0.0) for p in polarisation]
    mid = [float(getattr(p, "pct_mid", 0.0) or 0.0) for p in polarisation]
    high = [float(getattr(p, "pct_high", 0.0) or 0.0) for p in polarisation]
    x = np.arange(len(pillars))
    ax.bar(x, low, color=LOW_C, label="Low (1-4)", zorder=3)
    ax.bar(x, mid, bottom=low, color=MID_C, label="Mid (5-6)", zorder=3)
    ax.bar(x, high, bottom=[l + m for l, m in zip(low, mid)], color=HIGH_C, label="High (7-10)", zorder=3)
    for i in range(len(pillars)):
        if low[i] >= 12:
            ax.text(i, low[i] / 2, f"{low[i]:.0f}%", ha="center", va="center", fontsize=8, color="white", fontweight="bold")
        if high[i] >= 12:
            ax.text(i, low[i] + mid[i] + high[i] / 2, f"{high[i]:.0f}%", ha="center", va="center",
                    fontsize=8, color="white", fontweight="bold")
    ax.set_xticks(x); ax.set_xticklabels(pillars, fontsize=10, fontweight="bold")
    ax.set_ylim(0, 100); ax.set_ylabel("% of responses", fontsize=9)
    ax.set_title("Polarisation Profile — score distribution", fontsize=12.5, fontweight="bold", color=NAVY, pad=10)
...
def ranking_bar(all_depts, company_overall, focus=None, figsize=None, max_rows=None):
    """Dynamic height so many departments stay readable."""
    ordered = all_depts["Overall"].sort_values()
    if max_rows and len(ordered) > max_rows:
        # keep focus + top/bottom for compactness
